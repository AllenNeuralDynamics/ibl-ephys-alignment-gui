...
from ephys_alignment_gui.geometry.numeric import fcn_cosine
from ephys_alignment_gui.geometry.probe_geometry import TIP_SIZE_UM, trace_header

_logger = logging.getLogger(__name__)


def _extract_probes(sess_path: Path, *, bin_exists: bool) -> None:
    """Import the legacy ONE extraction helper only when that path is used."""
    from ibllib.ephys.spikes import probes_description as extract_probes

    extract_probes(sess_path, bin_exists=bin_exists)


def _qc_base():
    """Import the legacy QC helper only when online Alyx writes are used."""
...
def get_picked_tracks(
    histology_path, glob_pattern="*_pts_transformed.csv", brain_atlas=None
):
    """
    ...
    for ses in sessions:
        ...
        if not probe:
            ...
            if all([rf is None for rf in raw_files]):
                ...
                continue
            _extract_probes(sess_path, bin_exists=False)
...
def plot2d_all(trajectories, tracks, brain_atlas=None) -> None:
    """
...
def plot3d_all(trajectories, tracks, brain_atlas=None) -> None:
    """
    Plot all tracks on a single 2d slice
    :param trajectories: dictionary output of the Alyx REST query on trajectories
    :param tracks:
...
def register_chronic_track(
    ...
    for probe_id in chronic["probe_insertion"]:
        ...
        if picks is None or picks.size == 0:
            hist_qc = _qc_base().QC(pid, one=one, endpoint="insertions")
...
def register_track(
    ...
    if picks is None or picks.size == 0:
        ...
        hist_qc = _qc_base().QC(probe_id, one=one, endpoint=endpoint)
        hist_qc.update_extended_qc({"tracing_exists": False})
        hist_qc.update("CRITICAL", namespace="tracing")
    ...
    alyx_end = (
        "chronic_insertion" if endpoint == "chronic-insertions" else "probe_insertion"
    )
    hist_traj = one.alyx.get(
        f"/trajectories?&{alyx_end}={probe_id}&provenance=Histology track",
        clobber=True,
    )
...
def register_aligned_track(
    ...
    tdict = create_trajectory_dict(
        probe_id, insertion, provenance="Ephys aligned histology track"
    )
...
def register_chronic_track_files(
    path_tracks, one=None, overwrite=False, brain_atlas=None
) -> None:
    """
    ...
    for ind, track_file in enumerate(track_files):
        ...
        probe = one.alyx.rest(
            "chronic-insertions", "list", no_cache=True, **search_filter
        )
        if len(probe) == 0:
            raise ValueError(
                f"Could not find associated chronic insertion for {search_filter['subject']},"
...
def detect_missing_histology_tracks(
    path_tracks=None, one=None, subject=None, brain_atlas=None
) -> None:
    """
    ...
    for subj in unique_subjects:
        ...
        if len(insertions) != len(trajectories):
            ...
            traj_sess = np.array(
                [traj["session"]["id"] + traj["probe_name"] for traj in trajectories]
            )
...
def coverage(trajs, ba=None, dist_fcn=[100, 150]):
    ...
    def crawl_up_from_tip(ins, d):
        return (ins.entry - ins.tip) * (
            d[:, np.newaxis] / np.linalg.norm(ins.entry - ins.tip)
        ) + ins.tip
    ...
    for p in np.arange(len(trajs)):
        if len(trajs) > 20:
            if p % 20 == 0:
                _logger.info(
                    f"Processing trajectory {p}/{len(trajs)} ({100 * p / len(trajs):.1f}%)"
                )
        traj = trajs[p]
        ...
        ishank = np.round(
            np.array([np.linspace(tbi[0, i], tbi[1, i], nz) for i in np.arange(3)]).T
        ).astype(np.int32)
        ...
        # for those
        nx = (
            int(np.floor(MAX_DIST_UM / 1e6 / np.abs(ba.bc.dxyz[0]) * np.sqrt(2) / 2))
            * 2
            + 1
        )
        ny = (
            int(np.floor(MAX_DIST_UM / 1e6 / np.abs(ba.bc.dxyz[1]) * np.sqrt(2) / 2))
            * 2
            + 1
        )
        ixyz = np.stack(
