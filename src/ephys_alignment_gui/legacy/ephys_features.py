...
from ephys_alignment_gui.desktop.plot_elements import QRangeSlider, replace_axis
...
PLOT_TYPES = {
    "psd_delta": {"plot_type": "probe", "cmap": "viridis"},
...
class RegionFeatureWindow(QtWidgets.QMainWindow):
    def __init__(
        self, one, region_ids=None, ba=None, download=True, size=(1600, 800)
    ) -> None:
        super().__init__()
        ...
        if download:
            ...
            # Download file
            base_path = Path(
                "aggregates/atlas/latest"
            )  # TODO THIS FOLDER DOES NOT CONTAIN PROBES
            file_list = [
                "channels.pqt",
                "probes.pqt",
    ...
    def on_region_chosen(self, idx) -> None:
    ...
    def on_plot_chosen(self, idx) -> None:
    ...
    def clear_plots(self) -> None:
        for fig_hist, fig_feat, fig_cbar in zip(
            self.plots_hist, self.plots_feat, self.plots_cbar
        ):
            fig_hist.clear()
            fig_feat.clear()
            fig_cbar.clear()
    ...
    def get_idx(self):
        if self.page_idx == self.page_num:
            ...
            pid_idx = np.arange(
                self.page_idx * self.step,
                (self.page_idx * self.step) + self.step,
            )
    ...
    def plot_regions(self) -> None:
        ...
        for iD, dat in enumerate(data):
            ...
            for ir, (reg, col, reg_id) in enumerate(
                zip(dat["regions"], dat["colors"], dat["region_id"])
            ):
    ...
    def plot_line(self, fig_line, fig_cbar, data, offset) -> None:
        fig_line.clear()
        fig_cbar.clear()
    ...
    def plot_probe(self, fig_probe, fig_cbar, data, offset) -> None:
        fig_probe.clear()
        fig_cbar.clear()
    ...
    def get_feature_data(self, plot_name, plot_type) -> None:
        if plot_type == "probe":
    ...
    def arrange_channels2banks(self, data, chn_coords):
        ...
        for iX, x in enumerate(np.unique(chn_coords[:, 0])):
            ...
            # NP1.0 checkerboard
            bnk_full = np.arange(
                np.min(bnk_ycoords), np.max(bnk_ycoords) + bnk_diff, bnk_diff
            )
            ...
            _bnk_yscale = (chn_max - chn_min) / _bnk_data.shape[1]
