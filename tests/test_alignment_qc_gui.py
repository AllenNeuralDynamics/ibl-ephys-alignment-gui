...
class TestsAlignmentQcGUI(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        insertions = create_alyx_probe_insertions(
            session_path=EPHYS_SESSION,
            model="3B2",
            labels=probe,
            one=one,
            force=True,
        ...
        data = np.load(
            Path(
                Path(__file__).parent.joinpath("fixtures", "data_alignmentqc_gui.npz")
            ),
            allow_pickle=True,
        )
